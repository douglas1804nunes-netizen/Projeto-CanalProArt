...
| `TOKEN_ENCRYPTION_KEY`                                | Fase 4                  | Chave usada para criptografar `access_token`/`refresh_token` do YouTube antes de gravar no banco. Nunca reaproveitar o `JWT_SECRET` para isso. Quando definida, precisa decodificar para 32 bytes em hex ou base64 (AES-256-GCM) — validado em `src/env.ts`. |
